...
# load the penguins dataset from the local data directory
peng = pd.read_csv("../data/penglings.csv")

# build a scatter plot apping flipper length to body mass,
...
chart = (
    ...
    .encode(
        ...
        size=alt.Size(
            ...
            scale=alt.Scale(range=[40, 500])  # size range for bill lengthh
        )
    )
)
